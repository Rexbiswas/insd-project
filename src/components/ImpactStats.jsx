...
const ImpactStats = () => {
    ...
    return (
        <div ref={statsRef} className="width-full min-h-[50vh] bg-slate-900 flex items-center justify-center py-24 relative overflow-hidden">
            ...
            {/* Background Details */}
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-slate-800/50 via-slate-900 to-slate-950 pointer-events-none"></div>
            ...
            <div className="relative z-10 container mx-auto px-4 grid grid-cols-2 md:grid-cols-5 gap-12 text-center">
                {stats.map((stat, index) => (
                    <div key={index} className="stat-item group cursor-default flex flex-col items-center justify-center p-6 rounded-2xl hover:bg-white/5 transition-colors duration-300 border border-transparent hover:border-white/10">
                        <div
                            className={`stat-number text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-br ${stat.color} mb-4 group-hover:scale-110 transition-transform duration-500 inline-block drop-shadow-lg`}
